...
- [ ] 1.2 Add `micrometer-registry-prometheus` runtime dependency to `pom.xml`. Verify `mvnw.cmd -B -DskipTests package` succeeds.
- [ ] 1.3 Add the new package skeletons (empty `package-info.java`) under `infrastructure/metrics/`, `infrastructure/admin/`, `infrastructure/sync/`, `infrastructure/analytics/`, `application/analytics/`, `web/api/csv/`. ArchUnit must stay green.

### 2. Metrics — Requirement 1

- [ ] 2.1 Add `MetricsConfig` enabling `prometheus` actuator endpoint via YAML; expose `/actuator/prometheus`. Add an IT that asserts HTTP 200 and `Content-Type: text/plain; version=0.0.4`.
